...
class SerialController:
    ...
    def __init__(self, port_path, buadrate=9600):
        """
        初始化串口设置，初始化待发送指令
        :param port_path: linux--串口绝对路径；windows--COMXX，均为字符串
        :param buadrate: 波特率
        """
        self.ser = serial.Serial(port_path, buadrate)
        self.port = port_path
    ...
    def receive_encoder_data(self, timeout=0.5):
        """
        ...
        """
        FRAME_LEN = 12  # 数据帧长度
        recv_buffer = np.zeros(FRAME_LEN, dtype='uint8')
        time_start = time.time()
        time_run = 0
        while time_run < timeout:
            time_run = time.time() - time_start
            # 监听等待数据帧同步标志的到来: 0xA5 (数据帧开始)
            recv_buffer[0] = ord(self.ser.read(1))
            if recv_buffer[0] != 0xA5:
                continue

            # 已识别到数据帧，开始读取数据
            for t in range(1, FRAME_LEN):
                recv_buffer[t] = ord(self.ser.read(1))

            # 检查是否以结束符结束，是则正常，否则继续监听
            if recv_buffer[-1] != 0x5A:
                continue

            # 读取数据帧，帧包含三个数据
            # 左、右编码器累计脉冲数，各四个字节，x1.8后可作为厘米距离
            encoder_left = (recv_buffer[1] << 24) + (recv_buffer[2] << 16) \
                           + (recv_buffer[3] << 8) + recv_buffer[4]
            encoder_right = (recv_buffer[5] << 24) + (recv_buffer[6] << 16) \
                            + (recv_buffer[7] << 8) + recv_buffer[8]
            # 下位机系统时间，x100后可作为毫秒时间
            systime = (recv_buffer[9] << 8) + recv_buffer[10]
            return encoder_left, encoder_right, systime  # 正常退出
        # 长时间未获取到有效数据，抛出异常
        return None,None,None
        # raise Exception("Data receiver timeout. No valid data received.")
...
class MotorControl:
    def __init__(self,port,buadrate = 9600):
        ...
        self.sc = SerialController(port_path = port, buadrate= buadrate)

    def modifySpeed(self,LineSpeed,AngleSpeed,dirLeft = 1,dirRight = 0,disLeft = 20,disRight = 20):
        # 由线速度和角速度计算左右轮的速度
        vLeft = LineSpeed  # int(LineSpeed + 0.5 * AngleSpeed * self.WHEEL_DISTANCE)
        vRight = AngleSpeed  # int(LineSpeed - 0.5 * AngleSpeed * self.WHEEL_DISTANCE)
        # 左右轮符号不一样，就原地旋转
...
if __name__ == '__main__':
    # 初始化串口
    sc = SerialController(port_path='COM7', buadrate=9600)
    # 设置指令
